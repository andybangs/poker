...
describe('bestWildHand', () => {
  // The black joker, '?B', can be used as any spade or club
  // and the red joker, '?R', can be used as any heart or diamond
  it('should return the best five-card hand', () => {
    expect(
      bestWildHand('5C 6C 7C 8C 9C TC ?B'.split(' ')).sort()
    ).toEqual('7C 8C 9C TC JC'.split(' ').sort());
    expect(
      bestWildHand('TD TC 5H 5C 7C ?R ?B'.split(' ')).sort()
    ).toEqual('7C TD TC TH TS'.split(' ').sort());
    expect(
      bestWildHand('JD TC TH 7C 7D 7S 7H'.split(' ')).sort()
    ).toEqual('JD 7C 7D 7S 7H'.split(' ').sort());
  });
});
